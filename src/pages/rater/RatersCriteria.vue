...
// Update only the openRatingForm function in your PositionsList component
...
const openRatingForm = async (position) => {
  ...
  try {
    ...
    if (result && result.applicants?.length > 0) {
      ...
      console.log(`Processed ${modalApplicants.value.length} applicants for job ${position.id}:`, modalApplicants.value);
    } else {
