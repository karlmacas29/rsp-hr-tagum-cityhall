      ...
      <q-scroll-area style="height: 100vh;">
        ...
        <div v-if="selectedValue && structureTree.length > 0" class="q-mt-md">
          ...
          <div class="">
              ...
              >
                ...
                <template v-slot:default-header="prop">
                  <div class="row items-center cursor-pointer q-py-sm">
                    <q-icon :name="getNodeIcon(prop.node)" color="primary" size="sm" class="q-mr-sm" />
                    <div>{{ prop.node.label }}</div>
                    <q-badge v-if="prop.node.positions" color="green" class="q-ml-sm">
                      {{ prop.node.positions }} {{ prop.node.positions > 1 ? 'positions' : 'position' }}
                    </q-badge>
                  </div>
                </template>
              </q-tree>
            ...
            <!-- Details Panel
            <div v-if="selectedNodeData" class="col-12 col-md-4">
              <q-card flat bordered class="q-mt-md-none q-mt-md">
                <q-card-section>
                  <div class="text-h6">{{ selectedNodeData.label }}</div>
                  <q-badge :color="getNodeColor(selectedNodeData)">
                    {{ getNodeTypeName(selectedNodeData) }}
                  </q-badge>
                </q-card-section>
                ...
                <q-card-section>
                  <q-list dense>
                    <q-item v-if="selectedNodeData.data.office">
                      <q-item-section>
                        <q-item-label caption>Office</q-item-label>
                        <q-item-label>{{ selectedNodeData.data.office }}</q-item-label>
                      </q-item-section>
                    </q-item>
                    
                    <q-item v-if="selectedNodeData.data.division">
                      <q-item-section>
                        <q-item-label caption>Division</q-item-label>
                        <q-item-label>{{ selectedNodeData.data.division }}</q-item-label>
                      </q-item-section>
                    </q-item>
                    
                    <q-item v-if="selectedNodeData.data.section">
                      <q-item-section>
                        <q-item-label caption>Section</q-item-label>
                        <q-item-label>{{ selectedNodeData.data.section }}</q-item-label>
                      </q-item-section>
                    </q-item>
                    
                    <q-item v-if="selectedNodeData.data.unit">
                      <q-item-section>
                        <q-item-label caption>Unit</q-item-label>
                        <q-item-label>{{ selectedNodeData.data.unit }}</q-item-label>
                      </q-item-section>
                    </q-item>
                    
                    <q-item v-if="selectedNodeData.positions">
                      <q-item-section>
                        <q-item-label caption>Total Positions</q-item-label>
                        <q-item-label>{{ selectedNodeData.positions }}</q-item-label>
                      </q-item-section>
                    </q-item>
                  </q-list>
                </q-card-section>
        ...
        <div v-else-if="selectedValue && !usePlantilla.loading" class="text-center q-pa-lg">
          ...
          <div class="text-subtitle1 q-mt-sm">No organizational structure data available for this office</div>
        </div>
